...
4. 스코프 생성 시기
    ...
    ※ 스코프 체인은 “실행 컨텍스트의 렉시컬 환경”을 단방향으로 연결한 것이다.
    
    ※ 전역 렉시컬 환경 : “코드가 로드”되면 곧바로 생성
    
    ※ 햠수 렉시컬 환경 : “함수가 호출”되면 생성된다.
    (deep dive 195p)
...
- 지역 스코프의 생성시기는 런타임 이전이고, 지역 함수 선언은 런타임 이후 함수 호출 이전, 값 할당은 함수 실행하면서 코드 읽는 과정중인건지?<br/> 런타임 이전에 만들어진 “지역 함수 스코프”에는 어떤게 미리 들어가 있는건지 정리가 안됨